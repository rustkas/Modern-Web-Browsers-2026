...
В январе 2026 года Navigation API достиг статуса **Baseline Newly Available**, что означает его поддержку во всех современных браузерных движках: Chrome, Edge, Firefox 147 и Safari 26.2. Это событие ознаменовало конец эры, когда разработчикам SPA приходилось полагаться на устаревший History API, который Ian Hickson, бывший редактор HTML-спецификации, в интервью HTML5 Doctor назвал одной из своих «любимых ошибок» — говоря конкретно о том, что второй аргумент `pushState()` в итоге оказался невостребованным и фактически игнорируется браузерами.
...
### 2.5. Префетчинг и предзагрузка ресурсов (Speculation Rules API — не Baseline)

Navigation API не предоставляет встроенных методов префетчинга, но его архитектура отлично сочетается со стандартными механизмами предварительной загрузки — в первую очередь со **Speculation Rules API**. Прежде чем показывать код, стоит обозначить его реальный статус: по данным MDN, эта технология **не имеет статуса Baseline**. Production-ready реализация есть только в Chromium (Chrome/Edge, начиная с версии 108, с полным набором ключей синтаксиса — с 121); Firefox официально API не реализовал (команда лишь недавно заняла «positive» позицию по части prefetch, что означает намерение, а не готовую реализацию); Safari 26.2 содержит рабочую имплементацию, но она **отключена по умолчанию**. К счастью, API спроектирован как прогрессивное улучшение: браузеры без поддержки просто игнорируют тег `<script type="speculationrules">`, ничего не ломая, — так что использовать его безопасно, но рассчитывать на эффект в Firefox и Safari по умолчанию не стоит.
...
Обратите внимание: поскольку `precommitHandler` пока не поддерживается в Safari (см. раздел 2.2), а Speculation Rules API — только в Chromium, комбинация этих двух техник в связке даёт максимальный эффект именно в Chrome/Edge; в Firefox и Safari приложение продолжит корректно работать, но без ускорения от предзагрузки — это стоит закладывать как progressive enhancement, а не как гарантированное поведение.
...
Navigation API и View Transitions API вместе обеспечивают бесшовные анимации переходов между страницами. Напомним оговорку из Главы 3: same-document View Transitions (именно этот сценарий использования, разобранный ниже) — Baseline и работает во всех современных браузерах.
...
Navigation API в 2026 году представляет собой полноценную замену устаревшему History API, обеспечивая разработчиков инструментами для создания надежной, производительной маршрутизации в SPA — и это одна из наиболее зрелых и точно проверяемых технологий в книге: Baseline Newly Available с января 2026 года, во всех трёх основных движках, с единственной документированной оговоркой про `precommitHandler` в Safari:
...
- **Перехват навигации.** Метод `intercept()` с `precommitHandler` (загрузка данных до смены URL, пока не поддерживается в Safari) и `handler` (смена контента) обеспечивает полный контроль над навигацией.
- **Префетчинг через Speculation Rules API** — полезное, но пока не Baseline дополнение: production-ready только в Chromium, у Safari отключено по умолчанию, у Firefox нет реализации вовсе. Используйте как progressive enhancement.
...
- **Интеграция с View Transitions.** Анимации переходов между состояниями становятся простыми благодаря интеграции с View Transitions API (same-document — Baseline).
