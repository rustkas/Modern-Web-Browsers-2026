...
**Чтение данных из потока через async-итерацию (современный подход):**
...
for await (const chunk of reader) {
  console.log(chunk); // данные в верхнем регистре
}
```

### 1.4. Пайплайны: соединение потоков

Главная сила Streams API — возможность соединять потоки в пайплайны (pipe chains), где данные проходят через последовательность преобразований .
...
**Современный подход с async-итераторами (альтернатива pipeTo):**
...
В 2026 году **ошибки игнорирования backpressure остаются одной из главных причин утечек памяти в потоковых приложениях**, особенно в сложных системах с множеством вложенных потоков (как в Next.js или AWS SDK) .
...
**1. Стриминг и SEO-метаданные.** Современные фреймворки решают проблему, когда метаданные для SEO (`<title>`, `<meta>`), загружаемые асинхронно, не попадают в исходный HTML. Например, **Next.js 16** определяет, что запрос идет от бота (по User-Agent), и отключает стриминг, отправляя полный блокирующий ответ с полным `<head>` .
...
// Для ботов (Googlebot, Bingbot) — полный блокирующий HTML.
```

**2. Поддержка в фреймворках (2026):**
- **Next.js 16** — бот-осведомленный стриминг (по умолчанию для пользователей, блокировка для ботов) .
